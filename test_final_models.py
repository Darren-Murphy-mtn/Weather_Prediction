#!/usr/bin/env python3
"""
Final test script for Mount Rainier weather models
Ensures exact feature order matching
...
def create_features_with_exact_order(df, model):
    """
    ...
    Returns:
        DataFrame with features in exact order
    """
    print("🔧 Creating features with exact order...")
    ...
    # Create lag features (1, 2, 3 hours)
    ...
    # Create rolling averages (3, 6, 12 hours)
    for var in ['temperature_F', 'wind_speed_mph', 'air_pressure_hPa', 'precip_hourly']:
    ...
    # Create a new DataFrame with only the expected features in the exact order
    df_ordered = pd.DataFrame()
    for feature in expected_features:
        if feature in df_features.columns:
            df_ordered[feature] = df_features[feature]
        else:
            # If feature is missing, fill with 0
            df_ordered[feature] = 0
            print(f"   ⚠️ Missing feature: {feature}")
    
    print(f"   ✅ Created {len(df_ordered.columns)} features in exact order")
    return df_ordered
...
def evaluate_model_performance(models, df_features, year):
    ...
    for target_var in TARGET_VARIABLES:
        ...
        # Create features in exact order for this model
        X_test = create_features_with_exact_order(df_features, model)
...
def save_results(results, year):
    ...
    # Save to CSV
    results_file = PROCESSED_DATA_DIR / f"final_model_evaluation_{year}.csv"
...
def main():
    ...
    print("🧪 Mount Rainier Final Model Testing")
    print("=" * 60)
    ...
    for year in ['2015', '2016']:
        ...
        results = evaluate_model_performance(models, test_data, year)
